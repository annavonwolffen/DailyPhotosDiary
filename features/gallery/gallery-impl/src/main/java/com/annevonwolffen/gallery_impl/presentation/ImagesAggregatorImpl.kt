...
class ImagesAggregatorImpl @Inject constructor() : ImagesAggregator {
    override fun groupByDate(images: List<Image>): List<ImagesGroup> {
        return images.groupBy { it.date.toDateWithoutTime() }
            .map {
                ImagesGroup(it.key, it.value
                    .sortedWith(Comparator.nullsFirst(compareBy<Image> { image -> image.orderWithinDateGroup?.editTimeMillis }
                        .thenBy { image -> image.orderWithinDateGroup?.order }))
                    .map { image -> image.toPresentation() })
            }
    }
